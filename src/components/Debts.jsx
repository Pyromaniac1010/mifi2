...
import { computeFreedomDate, debtProgress, orderDebts } from '../lib/calculations';
import { CURRENCIES, money, convert } from '../lib/currency';

export default function Debts({ uid, debts, base, rates, debtStrategy }) {
  const [showAdd, setShowAdd] = useState(false);
  const freedom = computeFreedomDate(debts);
  const ordered = orderDebts(debts, debtStrategy, base, rates);
  const stratLabel = debtStrategy === 'snowball' ? 'Snowball' : 'Avalanche';
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500">Each debt is logged in its own currency; totals convert to your base ({base}). Ordered by your {stratLabel.toLowerCase()} strategy — change it in Settings.</p>
      ...
      {debts.length > 0 && freedom.date && (
        <div className="bg-gradient-to-r from-mint-500 to-navy-500 rounded-2xl p-6 text-white shadow-lg">
      ...
      {showAdd && <AddForm uid={uid} base={base} onDone={() => setShowAdd(false)} />}
      ...
      <div className="space-y-3">
        {debts.length === 0 ? (
          ...
          ordered.map((d, i) => <Card key={d.id} uid={uid} d={d} rank={i + 1} base={base} rates={rates} strategy={debtStrategy} />)
        )}
      </div>
    </div>
  );
...
function AddForm({ uid, base, onDone }) {
  ...
  const submit = async (e) => {
    ...
    setBusy(true);
    try {
      await addDebt(uid, { name: f.name.trim(), principal, interestRate, monthlyPayment, currency });
      onDone();
  ...
  return (
    <form onSubmit={submit} className="bg-white rounded-xl p-6 shadow-lg space-y-3">
      ...
      <div className="flex gap-2">
        <div className="flex-1"><Field label="Principal (current balance)"><input type="number" step="0.01" min="0" value={f.principal} onChange={set('principal')} className="input" placeholder="0.00" /></Field></div>
...
function Card({ uid, d, rank, base, rates, strategy }) {
  ...
  const [currency, setCurrency] = useState(d.currency || base);
  const set = (k) => (e) => setF({ ...f, [k]: e.target.value });
  ...
  const save = async () => {
    ...
    const monthlyPayment = parseFloat(f.monthlyPayment);
    if (!f.name.trim() || !(principal >= 0) || isNaN(interestRate) || !(monthlyPayment > 0)) return;
    await updateDebt(uid, d.id, { name: f.name.trim(), principal, interestRate, monthlyPayment, currency });
    setEditing(false);
  };
  ...
  if (editing) {
    return (
      <div className="bg-white rounded-xl p-5 shadow-lg border-2 border-navy-500 space-y-3">
        ...
        <div className="flex gap-2">
          <div className="flex-1"><Field label="Principal"><input type="number" step="0.01" min="0" value={f.principal} onChange={set('principal')} className="input" /></Field></div>
          <Field label="Currency"><CurrencySelect value={currency} onChange={setCurrency} /></Field>
        </div>
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-lg border-2 border-navy-100">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="bg-navy-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">{rank}</div>
          <div>
            <h3 className="font-bold text-gray-900 text-lg">{d.name}</h3>
            <p className="text-sm text-gray-500">{stratLabel} priority #{rank} · {cur}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <Box label="Principal" value={money(d.principal, cur)} bg="bg-gray-50" text="text-gray-900" />
        <Box label="Interest" value={`${d.interestRate}%`} bg="bg-red-50" text="text-red-600" />
        <Box label="Monthly" value={money(d.monthlyPayment, cur)} bg="bg-navy-50" text="text-navy-600" />
      </div>

      {cur !== base && (
        <p className="text-xs text-gray-400 -mt-2 mb-4">≈ {money(convert(d.principal, cur, base, rates), base)} balance · {money(convert(d.monthlyPayment, cur, base, rates), base)} monthly in {base}</p>
      )}

      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-gray-700">Progress</span>
          <span className="text-sm font-bold text-navy-600">{debtProgress(d).toFixed(1)}%</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div className="bg-gradient-to-r from-navy-600 to-mint-500 rounded-full h-3" style={{ width: `${debtProgress(d)}%` }} />
        </div>
      </div>

      {!paying ? (
        <button onClick={() => setPaying(true)} className="w-full bg-navy-600 text-white py-2 rounded-lg font-medium hover:bg-navy-700">Log Payment</button>
      ) : (
        <form onSubmit={pay} className="space-y-2">
          <input type="number" step="0.01" min="0" value={payAmount} onChange={(e) => setPayAmount(e.target.value)} className="input" placeholder={`Payment amount (${cur})`} autoFocus />
          <div className="flex gap-2">
            <button type="submit" className="flex-1 bg-mint-600 text-white py-2 rounded-lg font-medium">Confirm</button>
